...
if __name__ == "__main__":
       ...
       # for graph in ["ogbn-arxiv", "ogbn-products", "reddit", "ogbn-papers100M", "ogbl-citation2"]:
        for graph in ["ogbl-citation2"]:
            for partitioner in ["ldg", "spinner"]:
                t = [
                "python -m scripts.compute_partition_metrics_extern",
                    f"-graph /mnt/data/dgl/{graph}.dgl",
                    f"-vid2pid /mnt/data/partitioned/{graph}.{partitioner}.P{p}.vid2pid",
                    f"-cpp_metrics /mnt/data/edgelists/{graph}.directed.{partitioner}.{p}.edgecut.partitioning.metrics.json", 
                    f"-num_parts {p}",
    ...
    with open("scripts/compute_partition_metrics_extern_commands.sh", "w") as f:
        f.write("\n".join(commands))
