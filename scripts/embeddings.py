...
if __name__ == "__main__":
    ...
    results_dir_base = f"/mnt/data/gnn-partitioning/results/partitioning-metrics-fast/{timestamp}"
    ...
    def gnn_tamplate(graph_name_to_partition, graph_name_training, epochs, hidden, fanout, model_name):   
        l = len(fanout.split(" "))
        gnn_tamplate = [
            "python -m  partitioner.embeddings",
            "--mode mixed", 
    ...
    def feature_partitioning_tamplate(graph_name_to_partition, graph_name_training):   
        gnn_tamplate = [
            "python -m  partitioner.embeddings",
            "--mode mixed", 
             f"--model_name feature-partitioning", 
            "--partitioner feature-partitioning", 
            "--num_parts 2 4 8 16 32", 
          #  "--num_parts 2 4", 
            f"--dataset_to_partition {graph_name_to_partition}", 
            f"--dataset {graph_name_training}", 
    ...
    if True:
        ...
        EPOCHS = [0,1,2,3,4,5,10,30,70,90] #11
        # no 50, because we already did it above
        EPOCHS = [50] #1
    ...
    if False:
        for graph_name in GRAPHS:
            graph_name_to_partition = graph_name
            graph_name_training = graph_name
            commands.append(feature_partitioning_tamplate(
                graph_name_to_partition=graph_name_to_partition, 
                graph_name_training=graph_name_training))                     
    ...
    with open("scripts/embeddings.sh", "w") as f:
        f.write("\n".join(commands))
