    ...
    for epoch in range(args.num_epochs):
        ...
        with model.join():
            for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
               ...
               # print("Worker", g.rank(), "input_nodes", len(input_nodes), "seeds", len(seeds))
             ...
             #   print("Worker", g.rank(), "arrived at", time.time())
                loss.backward()
                compute_end = time.time()
                forward_time += forward_end - start
                backward_time += compute_end - backward_start
              #  print("Worker", g.rank(), "backward_time", backward_time)
                optimizer.step()
        ...
        part = g.rank()
        epoch_time = toc - tic
        sample_data_copy = sample_time
        forward = forward_time
        backward = backward_time
        update = update_time
        num_seeds_val = num_seeds
        num_inputs_val = num_inputs
        ...
        log_data = {
            ...
            "sample+data_copy": round(sample_data_copy, 4),
...
def main(args):
   ...
   # os.environ["GLOO_SOCKET_IFNAME"] = "enp66s0f0"
    #os.environ["TP_SOCKET_IFNAME"] = "enp66s0f0"
    #os.environ["DGL_INTERFACE"] = "enp66s0f0"
    #os.environ["MASTER_ADDR"] = "132.180.110.181"
    #os.environ["DGL_BIND_INTERFACE"] = "enp66s0f0" 
    #os.environ["MASTER_PORT"] = "1234"

    # Force DGL's RPC to bind explicitly to the external IP
    #os.environ["DGL_RPC_NETWORK"] = "socket"
    #os.environ["DGL_SERVER_IP"] = "132.180.110.181"
    ...
    if "trainer_id" in g.ndata:
        ...
        print("No trainer_id, use default")
        train_nid = dgl.distributed.node_split(
            g.ndata["train_mask"], pb, force_even=args.force_even
        )
        val_nid = dgl.distributed.node_split(
            g.ndata["val_mask"], pb, force_even=args.force_even
        )
        test_nid = dgl.distributed.node_split(
            g.ndata["test_mask"], pb, force_even=args.force_even
        )
    ...
    log_data = {
        "part": g.rank(),
        "train_nid": len(train_nid),
        "train_nid_local":  len(np.intersect1d(train_nid.numpy(), local_nid)),
        "val_nid": len(val_nid),
        "val_nid_local": len(np.intersect1d(val_nid.numpy(), local_nid)),
        "test_nid": len(test_nid),
        "test_nid_local": len(np.intersect1d(test_nid.numpy(), local_nid)),
